...
if __name__ == "__main__":
    base_lora_dir = "output/train/thermal/LLVIP__LLVIP/eps2"
    pretrained_ckpt = "ckpts/pretrained_weights_flash_atten.pt"
    log_base_dir = "output/param_stats"

    os.makedirs(log_base_dir, exist_ok=True)

    for subdir in sorted(os.listdir(base_lora_dir)):
        match = re.match(r"lora_r(\d+)_a([\d.]+)", subdir)
        if not match:
            continue
        rank, alpha = match.groups()
        lora_ckpt = os.path.join(base_lora_dir, subdir, "best_lora_weights.pt")
        if not os.path.exists(lora_ckpt):
            continue
        log_path = os.path.join(log_base_dir, f"lora_r{rank}_a{alpha}.log")
        print(f"\n🔍 Comparing: rank={rank}, alpha={alpha}")
        analyze_weights(pretrained_ckpt, lora_ckpt, log_path)
