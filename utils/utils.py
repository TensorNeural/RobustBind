...
def gen_label(logic, device):
    labels = torch.zeros_like(logic[0]).to(device)
    for i in range(labels.shape[0]):
        labels[i] = i
    labels = labels.to(torch.int64)
    return labels

def loss_fun(score, gold_score):
    probs = F.log_softmax(score, -1)
    loss = F.nll_loss(probs, gold_score, reduction='mean')
    return loss

def load_centre_embeddings(embeddings_path, device):
